...
class Network(object):
    ...
    def Bilstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            lstm_out, last_state = tf.compat.v1.nn.bidirectional_dynamic_rnn(
    ...
    def lstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            lstm_out, last_state = tf.compat.v1.nn.dynamic_rnn(lstm_cell, img,
    ...
    def proposal_layer(self, input, _feat_stride, anchor_scales, cfg_key, name):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            blob, bbox_delta = tf.compat.v1.py_func(proposal_layer_py,
                                          [input[0], input[1], input[2], cfg_key,
                                              _feat_stride, anchor_scales],
                                          [tf.float32, tf.float32])

            rpn_rois = tf.convert_to_tensor(value=tf.reshape(
                blob, [-1, 5]), name='rpn_rois')  # shape is (1 x H x W x A, 2)
            rpn_targets = tf.convert_to_tensor(
                value=bbox_delta, name='rpn_targets')  # shape is (1 x H x W x A, 4)
    ...
    def reshape_layer(self, input, d, name):
        ...
        if name == 'rpn_cls_prob_reshape':
            ...
            # reshape: (1, 2xA, H, W)
            # transpose: -> (1, H, W, 2xA)
            return tf.transpose(a=tf.reshape(tf.transpose(a=input, perm=[0, 3, 1, 2]),
                                           [input_shape[0],
                                            int(d),
                                            tf.cast(
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            if input_shape.ndims == 4:
                ...
                feed_in = tf.reshape(tf.transpose(
                    a=input, perm=[0, 3, 1, 2]), [-1, dim])
            else:
                feed_in, dim = (input, int(input_shape[-1]))

            if name == 'bbox_pred':
                init_weights = tf.compat.v1.truncated_normal_initializer(
                    0.0, stddev=0.001)
                init_biases = tf.compat.v1.constant_initializer(0.0)
            else:
                init_weights = tf.compat.v1.truncated_normal_initializer(
    ...
    def dropout(self, input, keep_prob, name):
        return tf.nn.dropout(input, 1 - (keep_prob), name=name)

    def l2_regularizer(self, weight_decay=0.0005, scope=None):
        def regularizer(tensor):
            with tf.compat.v1.name_scope(scope, default_name='l2_regularizer', values=[tensor]):
                l2_weight = tf.convert_to_tensor(value=weight_decay,
    ...
    def build_loss(self, ohem=False):
        ...
        rpn_loss_box_n = tf.reduce_sum(input_tensor=rpn_bbox_outside_weights * self.smooth_l1_dist(
            rpn_bbox_inside_weights * (rpn_bbox_pred - rpn_bbox_targets)), axis=[1])

        rpn_loss_box = tf.reduce_sum(
            input_tensor=rpn_loss_box_n) / (tf.reduce_sum(input_tensor=tf.cast(fg_keep, tf.float32)) + 1)
